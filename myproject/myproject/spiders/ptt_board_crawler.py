...
class PttBoardCrawlerSpider(scrapy.Spider):
    name = 'ptt_board_crawler'
    ...
    def parse(self, response):
        ...
        for div in main_list.findChildren('div', recursive=False):
            ...
            # self.logger.debug(f'{self.parse.__name__}: {class_name}')

            if class_name and 'r-list-sep' in class_name:
                self.logger.info('REACH LAST ARTICLE...')
                break

            if class_name and 'r-ent' in class_name:
                r_ent_title_a = div.find('div', class_='title').find('a')
                if not r_ent_title_a or not r_ent_title_a.has_attr('href'):
                    continue
                article_url = urljoin(self.host, r_ent_title_a['href'])
                article_title = r_ent_title_a.text
                self.logger.debug(f'Parse article: {article_url} {article_title}')
    ...
    def parse_article(self, response):
        ...
        if response.status != 200:
            self.logger.error('Error - {} is not available to access'.format(response.url))
            return
